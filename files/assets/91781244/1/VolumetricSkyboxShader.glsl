...
float noise( in vec3 x )
{
    ...
    vec2 rg = texture2D( texture_dithering, (uv+ 0.5)/256.0).yx;
...
vec3 sky( vec3 rd )
{
    ...
    col += 0.5*vec3(.09,.5,.0)*exp2(hort*8.-8.);
...
void getAlbedo()
{
    ...
    //sundir = vec3(cos(.53*iTime*0.1),0.5,sin(iTime*0.1));
